...
BPM_SOURCE = "osc"  # "osc": OSCのみ, "http": HTTPのみ, "both": 両方有効
...
async def connect():
    ...
    task0 = asyncio.create_task(tasks.midi_listner())
    background_tasks.append(task0)
    #OSCのみ
    if BPM_SOURCE in ["osc", "both"]:
        task1 = asyncio.create_task(state.oscserver.start())
    ...
    if BPM_SOURCE in ["http", "both"]:
        ...
        async def on_bpm_update(new_bpm):
            state.MasterBpm = new_bpm
            newBpm = state.MasterBpm * state.last_led_state["multiplier"]
            await state.led.change_bpm(newBpm)
            await tasks.send_data(1, 0, new_bpm)
        
        state.BPMCient.on_bpm_change(on_bpm_update)
        await state.BPMCient.start()
